...
function loginStatusHandler(isLogin) {
    isLogin || $("#userBtn").html('登录/注册')
}
function login() {
    let user = $("#userInput").val(), pwd = $("#pwdInput").val();
    if (!user || !pwd) {
        return $.confirm({
            theme: "bootstrap",
            type: "red",
            title: "提示",
            content: '<span class="fileitemTr">' + '用户和密码不能为空！' + "</span>",
    ...
    $.ajax(settings).done((function(data) {
        $("#userBtn").html('<i style="color:ForestGreen">' + user + "</i>"),
        $('#userRank td:nth-child(1)').text(user),
        ...
        401 === jqXHR.status ? loginStatusHandler(!1) : $.confirm({
            theme: "bootstrap",
            type: "red",
            title: "提示",
            content: '<span class="fileitemTr">' + '用户或密码错误，请重新输入' + "</span>",
            autoClose: "cancelAction|1200",
            buttons: {
                cancelAction: {
                    text: '关闭',
...
function regist() {
    ...
    $.ajax(settings).done((function(data) {
        ...
        401 === jqXHR.status ? loginStatusHandler(!1) : $.confirm({
            ...
            type: "red",
            title: "提示",
            content: '<span class="fileitemTr">' + '用户名已存在, 请修改后重试!' + "</span>",
...
function refreshRank() {
    ...
    $.ajax(settings).done((function(data) {
        ...
        401 === data.status && $.confirm({
            ...
            type: "red",
            title: "提示",
            content: '<span class="fileitemTr">' + '请登录后查看排名。' + "</span>",
            autoClose: "cancelAction|1200",
            buttons: {
                cancelAction: {
                    text: '确定',
...
$("#userBtn").on("click", (function() {
    $("#userBtn").text() === '登录/注册' ? ($("#loginModal").modal("toggle"),
    setTimeout((function() {
        $("#userInput").focus(),
        $("#userInput").select(),
        $("#pwdInput").val("")
        ...
        buttons: {
            confirm: {
                ...
                action: function() {
                    ...
                    $.ajax(settings).done((function(data) {
                        $("#userBtn").html("登录/注册");
                        $('#userRank td:nth-child(1)').text('-');
                        $('#userRank td:nth-child(2)').text('-');
                    }
                    )).fail((function(jqXHR) {
                        $.confirm({
                            theme: "bootstrap",
...
$("#userInput").on("keypress", (function(event) {
    "13" == event.keyCode && $("#pwdInput").focus()
}
)),
...
function submitScore(score) {
    ...
    $.ajax(settings).done((function(data) {})).fail((function(jqXHR) {}));
}
(function() {
